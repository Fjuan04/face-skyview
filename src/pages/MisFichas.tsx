...
export default function MisFichas() {
  ...
  const fetchAttendanceReport = async () => {
    ...
    try {
      const res = await api.get(`/management/attendance/report?group_id=${selectedGroupId}&start_date=${startDate}&end_date=${endDate}`);
      if (res?.success) {
        setReportData({
          columns: res.columns,
          rows: res.rows
  ...
  useEffect(() => {
    if (activeTab === 'attendance' && selectedGroupId) {
              ...
              <section className="lg:col-span-8 flex flex-col gap-6">
                <AnimatePresence mode="wait">
                  {!selectedGroupId ? (
                    ...
                    >
                      <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md flex flex-col items-start gap-6">
                        <div className="w-full flex flex-col md:flex-row items-center justify-between gap-6">
                            ...
                            {/* TABS SELECTOR */}
                            <div className="flex p-1 bg-white/5 rounded-xl border border-white/10 self-center md:self-auto">
                                <button 
                                    onClick={() => setActiveTab('members')}
                                    className={`px-4 py-2 rounded-lg text-xs font-bold font-plus transition-all flex items-center gap-2 ${activeTab === 'members' ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/20' : 'text-white/40 hover:text-white/60'}`}
                                >
                                    <Users size={14} /> Aprendices
                                </button>
                                <button 
                                    onClick={() => setActiveTab('attendance')}
                                    className={`px-4 py-2 rounded-lg text-xs font-bold font-plus transition-all flex items-center gap-2 ${activeTab === 'attendance' ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/20' : 'text-white/40 hover:text-white/60'}`}
                                >
                                    <FileSpreadsheet size={14} /> Asistencia
                                </button>
                            </div>
                        </div>
                        ...
                        {activeTab === 'attendance' && (
                            <div className="w-full flex flex-col md:flex-row items-end gap-4 bg-white/[0.02] p-4 rounded-xl border border-white/5">
                                <div className="flex-1 w-full space-y-2">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-sm text-white focus:outline-none [color-scheme:dark]"
                                    />
                                </div>
                                <div className="flex-1 w-full space-y-2">
                                    <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold flex items-center gap-2">
                                ...
                                <button 
                                    ...
                                    className="px-6 py-2.5 bg-blue-500 hover:bg-blue-600 disabled:opacity-50 text-white rounded-xl text-xs font-bold font-plus transition-all flex items-center gap-2 whitespace-nowrap"
                                >
                                    {loadingReport ? <Loader2 className="animate-spin" size={14} /> : <Search size={14} />}
                                    Consultar
                                </button>
                            </div>
                        )}
                        ...
                        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-md">
                            {loadingReport ? (
                                <div className="py-40 flex flex-col items-center justify-center gap-4">
                                    ...
                                    <p className="text-white/40 text-xs font-plus animate-pulse">Generando reporte de asistencia...</p>
                                </div>
                            ) : !reportData ? (
                                <div className="py-40 flex flex-col items-center justify-center gap-4">
                                    <FileSpreadsheet className="text-white/5" size={48} />
                                    <p className="text-white/20 text-sm font-plus italic">Utiliza los filtros arriba para consultar la asistencia.</p>
                                </div>
                            ) : (
                                <div className="relative">
                                    <div className="overflow-x-auto custom-scrollbar" style={{ maxHeight: 'calc(100vh - 450px)' }}>
                                        <table className="w-full border-collapse">
                                            <thead>
                                                <tr className="bg-blue-500/10 border-b border-white/10">
                                                    <th className="sticky left-0 z-20 bg-slate-900/90 backdrop-blur-md border-r border-white/10 px-4 py-3 text-left text-[10px] uppercase tracking-widest font-bold text-white/60 min-w-[200px]">
                                                        Aprendiz
                                                    </th>
                                                    {reportData.columns.slice(1).map((col, idx) => (
                                                        <th key={idx} className="px-4 py-3 text-center text-[10px] uppercase tracking-widest font-bold text-white/60 border-r border-white/5 whitespace-nowrap min-w-[100px]">
                                                            {col}
                                                        </th>
                                                    ))}
                                                </tr>
                                            ...
                                            <tbody>
                                                {reportData.rows.map((row, rowIdx) => (
                                                    <tr key={rowIdx} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors group">
                                                        <td className="sticky left-0 z-10 bg-slate-900/90 backdrop-blur-md border-r border-white/10 px-4 py-3 text-sm font-bold text-white group-hover:text-blue-400 transition-colors font-plus truncate">
                                                            {row.nombre}
                                                        </td>
                                                        {row.sessions.map((value, valIdx) => (
                                                            <td key={valIdx} className="px-4 py-3 text-center border-r border-white/5">
                                                                <span className={`inline-flex items-center justify-center w-8 h-8 rounded-lg text-xs font-mono font-bold ${
                                                                    value > 0 
