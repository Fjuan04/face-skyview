...
export default function ExportAttendanceModal({ 
  ...
  const handleExport = async () => {
    ...
    try {
      const filename = `Asistencia_Ficha_${groupId}_${startDate}_a_${endDate}.xlsx`;
      // Endpoint: GET management/attendance/export?group_id=X&start_date=YYYY-MM-DD&end_date=YYYY-MM-DD
      await api.download(`/management/attendance/export?group_id=${groupId}&start_date=${startDate}&end_date=${endDate}`, filename);
      onClose();
    } catch (err: any) {
      console.error("Attendance export error:", err);
      setError(err?.message || "No se pudo generar el reporte de asistencia. Verifica el rango de fechas.");
    } finally {
          ...
          <div className="relative p-6 border-b border-white/5 bg-white/[0.02]">
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-white tracking-tight text-shadow-glow shadow-blue-500/10">Exportar Asistencia</h2>
              </div>
              <p className="text-xs text-white/40 mt-1">Selecciona el rango de fechas para el reporte Excel de esta ficha.</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 gap-5">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[10px] uppercase tracking-[0.15em] font-bold text-white/40 ml-1 flex items-center gap-2">
                  <Calendar size={12} className="text-blue-400/50" /> Fecha Inicial
                </label>
                <input
                  type="date"
                  value={startDate}
                  onChange={(e) => setStartDate(e.target.value)}
                  className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all w-full [color-scheme:dark]"
                />
              </div>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[10px] uppercase tracking-[0.15em] font-bold text-white/40 ml-1 flex items-center gap-2">
                  <Calendar size={12} className="text-blue-400/50" /> Fecha Final
                </label>
                <input
                  type="date"
                  value={endDate}
            ...
            <button
              ...
              className={`w-full py-3.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 shadow-lg active:scale-[0.98] border
                ${isExporting 
                  ...
                  : 'bg-blue-500 hover:bg-blue-400 border-blue-400/20 text-white'
                }
              `}
            >
              {isExporting ? (
